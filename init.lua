...
local BLIGHTFORGED_PLATE_PIECES = {
    'Plate Helmet of the Blightforged Warlord',
    'Breastplate of the Blightforged Warlord',
    'Plate Armguards of the Blightforged Warlord',
    'Legplates of the Blightforged Warlord',
    'Plate Handguards of the Blightforged Warlord',
    'Plate Wristguards of the Blightforged Warlord',
    'Plate Sabatons of the Blightforged Warlord',
}
local ROTFANG_CHAIN_PIECES = {
    'Chain Helmet of the Rotfang Hunter',
    'Chain Armguards of the Rotfang Hunter',
    'Chain Handguards of the Rotfang Hunter',
    'Chain Sabatons of the Rotfang Hunter',
    'Chain Hauberk of the Rotfang Hunter',
    'Chain Legguards of the Rotfang Hunter',
    'Chain Wristguards of the Rotfang Hunter',
}
local MOORSHADE_SILK_PIECES = {
    'Silk Crown of the Moorshade Magus',
    'Silk Sleeves of the Moorshade Magus',
    'Silk Gloves of the Moorshade Magus',
    'Silk Wristguards of the Moorshade Magus',
    'Silk Slippers of the Moorshade Magus',
    'Silk Robes of the Moorshade Magus',
    'Silk Leggings of the Moorshade Magus',
}
local BLIGHTCLAW_LEATHER_PIECES = {
    'Leather Hood of the Blightclaw Stalker',
    'Leather Sleeves of the Blightclaw Stalker',
    'Leather Grasp of the Blightclaw Stalker',
    'Leather Boots of the Blightclaw Stalker',
    'Leather Vest of the Blightclaw Stalker',
    'Leather Leggings of the Blightclaw Stalker',
    'Leather Bindings of the Blightclaw Stalker',
}
...
local function component_maps_equal(left, right)
    if left == right then
        return true
    end
...
local function get_sorted_progress_rows(mode, sort_specs, slot_name)
    ...
    local function compare_fn(left, right)
        if mode == 'peer_summary' then
            ...
            if column_index == (show_faction and 6 or 4) then
                return compare_tier_11_deficits(left, right, 'major', ascending)
            end
            if column_index == (show_faction and 7 or 5) then
                return compare_tier_11_deficits(left, right, 'minor', ascending)
            end
            if column_index == (show_faction and 8 or 6) then
...
local function display_gui()
    ...
    if show then
        ...
        if previous_tier ~= selected_tier then
            ...
            triggers.do_refresh = true
            if selected_tier == 'Tier 10' then
                request_plagueborn_refresh('tier_change')
            end
        end
        if selected_tier == 'Tier 10' then
            ImGui.SameLine()
        ...
        if #state.peer_order > 0 then
            ImGui.Separator()
            colored_text(COLOR_HEADER, 'Peer Details')
            for _, peer_name in ipairs(state.peer_order) do
                local peer_progress = state.peer_progress[peer_name]
                if ImGui.CollapsingHeader(peer_name .. '##peer_' .. peer_name) then
                    render_progress_block(peer_progress, false)
                end
            end
        end
    end
...
local function main()
    ...
    while running do
        ...
        if mq.gettime() >= next_plagueborn_poll_at then
            if refresh_plagueborn_progress() then
                pending_publish_tier = 'Tier 10'
                pending_publish_tier_10_faction = state.selected_tier_10_faction or DEFAULT_TIER_10_FACTION
                triggers.need_publish = true
