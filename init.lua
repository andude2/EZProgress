...
local EZPROGRESS_DATA = require('EZProgress.tier_data')
local DEFAULT_TIER_KEY = EZPROGRESS_DATA.default_tier_key or 'Tier 12'
local TIER_CONFIGS = EZPROGRESS_DATA
local TIER_ORDER = EZPROGRESS_DATA.tier_order or { DEFAULT_TIER_KEY }
local CLASS_TO_ARMOR_TYPE = EZPROGRESS_DATA.class_to_armor_type or {}
local FACTION_TARGETS = EZPROGRESS_DATA.faction_targets or {}
local FACTION_RANK_TO_SLOT = EZPROGRESS_DATA.faction_rank_to_slot or {}
local TIER_10_CONFIG = TIER_CONFIGS['Tier 10'] or {}
local TIER_11_CONFIG = TIER_CONFIGS['Tier 11'] or {}
local DEFAULT_TIER_10_FACTION = TIER_10_CONFIG.default_faction or 'Faydwer Alliance'
local TIER_10_FACTION_CHOICES = TIER_10_CONFIG.faction_choices or {}
local TIER_11_COLD_BARGAIN = TIER_11_CONFIG.cold_bargain or {}
...
local function get_tier_component_config(tier_key, armor_type)
    local tier_config = get_tier_config(tier_key)
    if not tier_config then
        return nil
    end

    if tier_config.component_names and tier_config.component_requirements then
        return {
            component_names = tier_config.component_names,
            component_requirements = tier_config.component_requirements,
            component_labels = tier_config.component_names,
        }
    end

    if tier_config.component_names_by_armor_type and tier_config.component_requirements_by_armor_type
        and tier_config.component_names_by_armor_type[armor_type] then
        return {
            component_names = tier_config.component_names_by_armor_type[armor_type],
            component_requirements = tier_config.component_requirements_by_armor_type[armor_type],
            component_labels = tier_config.component_names_by_armor_type[armor_type],
        }
    end
    return nil
end
...
local function get_tier_10_set_config(armor_type, tier_10_faction_key)
    ...
    local armor_templates = TIER_10_CONFIG.sets_by_armor_type and TIER_10_CONFIG.sets_by_armor_type[armor_type]
    local template = armor_templates and armor_templates[active_faction]
    if template then
        return {
